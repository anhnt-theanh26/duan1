...
function select_new()
{
    $sql = "SELECT danhmuc.ten_danh_muc, danhmuc.id as iddm, tintuc.tieu_de, tintuc.img_tin_tuc, tintuc.ngay_dang, tintuc.id as idtt FROM tintuc JOIN danhmuc ON tintuc.iddm=danhmuc.id WHERE tintuc.trang_thai = '0' and danhmuc.trang_thai = '0' ORDER BY tintuc.id DESC LIMIT 0,3";
    $list = pdo_query($sql);
    return $list;
}
...
{
    $sql = "SELECT danhmuc.ten_danh_muc, danhmuc.id as iddm, tintuc.tieu_de, tintuc.img_tin_tuc, tintuc.ngay_dang, tintuc.id as idtt FROM tintuc JOIN danhmuc ON tintuc.iddm=danhmuc.id WHERE tintuc.trang_thai = '0' and danhmuc.trang_thai = '0' ORDER BY tintuc.id DESC";
    $list = pdo_query($sql);
    return $list;
}
